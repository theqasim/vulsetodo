...
export const addItemToList = async (listId: string, itemName: string) => {
  const response = await api.post(`/todo-lists/${listId}/items`, { name: itemName });
  return response.data;
};
...
export const updateTodoItemStatus = async (listId: string, itemId: string, completed: boolean) => {
  const response = await api.patch(`/todo-lists/${listId}/items/${itemId}`, { completed });
  return response.data;
};